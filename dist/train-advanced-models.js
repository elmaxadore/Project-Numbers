...
function extractFeatures(matches) {
    const dataset = [];
    // Group by league and season for form calculation
    const grouped = {};
    for (const match of matches) {
        if (match.sport !== 'football')
            continue; // Only process football for now
    ...
    for (const [key, leagueMatches] of Object.entries(grouped)) {
        ...
        for (const match of sortedMatches) {
            ...
            const homeGoalsAvg = teamStats[homeTeam].goalsFor.length > 0
                ? teamStats[homeTeam].goalsFor.reduce((a, b) => a + b, 0) / teamStats[homeTeam].goalsFor.length
                : 1.5;
            const awayGoalsAvg = teamStats[awayTeam].goalsFor.length > 0
                ? teamStats[awayTeam].goalsFor.reduce((a, b) => a + b, 0) / teamStats[awayTeam].goalsFor.length
                : 1.5;
            const homeConcededAvg = teamStats[homeTeam].goalsAgainst.length > 0
                ? teamStats[homeTeam].goalsAgainst.reduce((a, b) => a + b, 0) / teamStats[homeTeam].goalsAgainst.length
                : 1.5;
            const awayConcededAvg = teamStats[awayTeam].goalsAgainst.length > 0
                ? teamStats[awayTeam].goalsAgainst.reduce((a, b) => a + b, 0) / teamStats[awayTeam].goalsAgainst.length
                : 1.5;
            const homeFormPoints = homeForm.filter((r) => r === 'W').length * 3 + homeForm.filter((r) => r === 'D').length;
            const awayFormPoints = awayForm.filter((r) => r === 'W').length * 3 + awayForm.filter((r) => r === 'D').length;
            const homeShotsAvg = teamStats[homeTeam].shots.length > 0
                ? teamStats[homeTeam].shots.reduce((a, b) => a + b, 0) / teamStats[homeTeam].shots.length
                : 12;
            const awayShotsAvg = teamStats[awayTeam].shots.length > 0
                ? teamStats[awayTeam].shots.reduce((a, b) => a + b, 0) / teamStats[awayTeam].shots.length
                : 12;
            const features = [
                homeGoalsAvg,
                awayGoalsAvg,
            ...
            if (match.homeScore !== undefined && match.awayScore !== undefined) {
                ...
                if (match.homeShots)
                    teamStats[homeTeam].shots.push(match.homeShots);
                if (match.awayShots)
                    teamStats[awayTeam].shots.push(match.awayShots);
                // Result for form
...
async function main() {
    ...
    const bestModel = models.reduce((best, curr) => curr.metrics.auc > best.metrics.auc ? curr : best);
...
//# sourceMappingURL=train-advanced-models.js.map