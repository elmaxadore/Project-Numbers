...
export class RealDataCollector {
    cacheDir;
    useCache;
    constructor(cacheDir = './data-cache', useCache = true) {
        ...
        for (const league of leagues) {
            for (const season of seasons) {
                try {
                    ...
                    console.warn(`Failed to fetch ${league.name} ${season}:`, error.message);
                }
            }
        }
    ...
    async scrapeBasketballData() {
        ...
        for (const year of years) {
            try {
                ...
                console.warn(`Failed to fetch NBA ${year}:`, error.message);
            }
        }
        await this.setCache(cacheKey, matches);
    ...
    async scrapeBaseballData() {
        ...
        for (const year of years) {
            try {
                ...
                console.warn(`Failed to fetch MLB ${year}:`, error.message);
            }
        }
        await this.setCache(cacheKey, matches);
    ...
    async scrapeHockeyData() {
        ...
        for (const year of years) {
            try {
                ...
                console.warn(`Failed to fetch NHL ${year}:`, error.message);
            }
        }
        await this.setCache(cacheKey, matches);
     ...
     */
    calculateTeamForm(matches, team) {
        const teamMatches = matches.filter(m => m.homeTeam === team || m.awayTeam === team).sort((a, b) => b.date.getTime() - a.date.getTime());
        const form = [];
...
//# sourceMappingURL=real-data-collector.js.map