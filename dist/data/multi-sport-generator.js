import { apiFetcher } from './sports-scraper.js';
export class MultiSportDataGenerator {
     ...
     */
    async generateHistoricalData(seasons = 3, matchesPerSeason = 500) {
        ...
        for (const sport of this.sports) {
            ...
            const sportMatches = this.generateSportSpecificData(sport, seasons, matchesPerSeason);
     ...
     */
    generateSportSpecificData(sport, seasons, matchesPerSeason) {
        const matches = [];
        const now = new Date();
        switch (sport) {
            case 'football':
    ...
    generateFootballData(seasons, matchesPerSeason, now) {
        const matches = [];
        const leagues = [
    ...
    generateBasketballData(seasons, matchesPerSeason, now) {
        const matches = [];
        const teams = [
    ...
    generateTennisData(seasons, matchesPerSeason, now) {
        const matches = [];
        const players = [
    ...
    generateBaseballData(seasons, matchesPerSeason, now) {
        const matches = [];
        const teams = [
    ...
    generateHockeyData(seasons, matchesPerSeason, now) {
        const matches = [];
        const teams = [
     ...
     */
    convertRealToMultiSport(realData) {
...
//# sourceMappingURL=multi-sport-generator.js.map