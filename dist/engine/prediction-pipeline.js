...
import { extractFeatures, predict, DEFAULT_O25_WEIGHTS, DEFAULT_BTTS_WEIGHTS, } from './logistic-regression.js';
import { filterWithValueEdge, } from './value-calculator.js';
import { generateAllTickets, outputTickets, } from './action-trigger.js';
import { logger } from '../utils/logger.js';
/**
 * Generate predictions for a fixture using the trained models
 */
export function predictFixture(fixtureData, o25Model = DEFAULT_O25_WEIGHTS, bttsModel = DEFAULT_BTTS_WEIGHTS) {
    const stats = fixtureData.expectedStats;
    const predictions = [];
    // Extract feature vector
    const features = extractFeatures(stats.homeTeamStats.xG, stats.homeTeamStats.xGA, stats.awayTeamStats.xG, stats.awayTeamStats.xGA, stats.homeTeamStats.cleanSheetRate, stats.awayTeamStats.failedToScoreRate, stats.fixtureId % 100, stats.homeTeamStats.avgGoalsScored, stats.homeTeamStats.avgGoalsConceded, stats.awayTeamStats.avgGoalsScored, stats.awayTeamStats.avgGoalsConceded);
 ...
 */
export async function runPipeline(fixturePackages, o25Model = DEFAULT_O25_WEIGHTS, bttsModel = DEFAULT_BTTS_WEIGHTS, config = CONFIG) {
