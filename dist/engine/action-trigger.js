 ...
 */
export function generateBettingTicket(pkg, prediction, valueAnalysis, config = CONFIG) {
    const stakeInfo = calculateStake(valueAnalysis.modelProbability, valueAnalysis.bestOdds, config.bankroll, config.kellyFractionCap, config.fixedStakePercentage);
 ...
 */
export function generateAllTickets(qualifiedBets, config = CONFIG) {
    const tickets = [];
 ...
 */
export function outputTickets(tickets) {
    if (tickets.length === 0) {
 ...
 */
export function exportTicketsJSON(tickets, filePath) {
    const output = {
