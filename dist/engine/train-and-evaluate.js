...
// ============================================================
import { generateSyntheticTrainingData, splitData, kFoldCrossValidation, createEnsembleModel, evaluateModel, } from './model-trainer.js';
import { DEFAULT_O25_WEIGHTS, DEFAULT_BTTS_WEIGHTS, saveModelToFile, } from './logistic-regression.js';
import { runBacktest, printBacktestResults } from './backtesting/index.js';
import { logger } from '../utils/logger.js';
/**
 * Generate mock historical fixtures for testing
 */
function generateMockHistoricalFixtures(count = 100) {
    const fixtures = [];
    const leagues = [
...
╚══════════════════════════════════════════════════════╝
  `);
    ...
    const o25ValData = val.map(d => ({
        features: d.features,
        label: d.o25Label,
    }));
    const bttsValData = val.map(d => ({
        features: d.features,
        label: d.bttsLabel,
    }));
...
//# sourceMappingURL=train-and-evaluate.js.map