...
// ============================================================
import { trainModel, predict, } from './logistic-regression.js';
 ...
 */
export function generateSyntheticTrainingData(numSamples = 2000, seed) {
    const data = [];
 ...
 */
export function splitData(data, trainRatio = 0.8, seed) {
    let random = Math.random;
    if (seed !== undefined) {
        let s = seed;
        random = () => {
...
export function evaluateModel(model, testData) {
    ...
    for (const sample of testData) {
        ...
        if (pred === sample.label)
            correct++;
        if (pred === 1 && sample.label === 1)
            tp++;
        else if (pred === 0 && sample.label === 0)
            tn++;
        else if (pred === 1 && sample.label === 0)
            fp++;
        else if (pred === 0 && sample.label === 1)
            fn++;
    }
    ...
    const buckets = new Array(calibrationBuckets).fill(null).map(() => ({ sumProb: 0, sumActual: 0, count: 0 }));
 ...
 */
export function trainWithTuning(trainData, valData, label) {
 ...
 */
export function kFoldCrossValidation(data, k = 5, label) {
    ...
    for (let fold = 0; fold < k; fold++) {
        ...
        const trainSamples = trainData.map(d => ({
            features: d.features,
            label: label === 'o25' ? d.o25Label : d.bttsLabel,
        }));
        const valSamples = valData.map(d => ({
            features: d.features,
            label: label === 'o25' ? d.o25Label : d.bttsLabel,
        }));
 ...
 */
export function createEnsembleModel(models, label) {
    if (models.length === 0) {
 ...
 */
export function predictEnsemble(features, models) {
