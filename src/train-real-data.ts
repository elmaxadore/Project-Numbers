...
async function trainWithRealData() {
  ...
  for (const sport of sports) {
    ...
    const labelsO25: number[] = []; // Over 2.5 goals / equivalent
    const labelsMoneyline: number[] = []; // Home win

    for (const match of sportMatches) {
      const featObj = featureExtractor.extractFeatures(match as any);
      
      // Convert SportFeatures object to array of numbers
      const featureArray = Object.values(featObj).filter(v => typeof v === 'number' && !isNaN(v)) as number[];
    ...
    const accO25 = predO25.reduce((sum: number, p: number, i: number) => 
      sum + (Math.round(p) === labelsO25[i] ? 1 : 0), 0) / labelsO25.length;
    ...
    const accML = predML.reduce((sum: number, p: number, i: number) => 
      sum + (Math.round(p) === labelsMoneyline[i] ? 1 : 0), 0) / labelsMoneyline.length;
