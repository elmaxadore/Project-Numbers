...
export class AdvancedTimeSeriesTrainer {
  ...
  private featureExtractor: MultiSportFeatureExtractor;
  ...
  ): { train: HistoricalMatch[]; test: HistoricalMatch[] } {
    // Sort by date
  ...
  private timeSeriesCV(
    ...
    nSplits: number = 5,
    gapDays: number = 30
  ): Array<{ train: HistoricalMatch[]; test: HistoricalMatch[] }> {
    const sorted = [...matches].sort((a, b) => a.date.getTime() - b.date.getTime());
    const splits: Array<{ train: HistoricalMatch[]; test: HistoricalMatch[] }> = [];
    ...
    for (let i = 0; i < nSplits; i++) {
      ...
      const test = sorted.filter(
        m => m.date.getTime() >= testStartTime && m.date.getTime() < testEndTime
      );
  ...
  private extractFeaturesWithTemporalAwareness(
    matches: HistoricalMatch[],
    sport: string
  ): { features: number[][]; labels: { overUnder: number[]; moneyline: number[] }; matchDates: Date[] } {
    const features: number[][] = [];
    const labelsO25: number[] = [];
    const labelsML: number[] = [];
  ...
  private updateTeamStats(
    stats: Map<string, { matches: number[]; goalsFor: number[]; goalsAgainst: number[]; form: ('W' | 'D' | 'L')[] }>,
    team: string,
    goalsFor: number,
    goalsAgainst: number,
    isHome: boolean
   ...
   */
  private calculateMetrics(predictions: number[], actuals: number[], probabilities: number[]): {
    accuracy: number;
    precision: number;
    recall: number;
    f1Score: number;
    ...
    for (let b = 0; b < buckets; b++) {
      ...
      const bucketActuals = actuals.filter((_, i) => 
        probabilities[i] >= b / buckets && probabilities[i] < (b + 1) / buckets
      );
   ...
   */
  async trainAllSports(): Promise<ModelEvaluation[]> {
    ...
    const allEvaluations: ModelEvaluation[] = [];
    ...
    for (const sport of sports) {
      ...
      for (const market of markets) {
        const labelKey = market as keyof typeof labels;
        const labelData = labels[labelKey];
        
        if (labelData.length < 100) continue;
        ...
        for (const modelName of models) {
          ...
          for (let fold = 0; fold < cvSplits.length; fold++) {
            ...
            if (trainData.features.length < 50 || testData.features.length < 20) continue;
            ...
            try {
              const result = await this.trainAndEvaluateModel(
                trainFeatures, trainLabels, testFeatures, testLabels, modelName
              );
              ...
              console.warn(`    Fold ${fold} failed: ${(error as Error).message}`);
            }
          }
          ...
          if (cvResults.length > 0) {
            ...
            const stdAccuracy = Math.sqrt(
              cvResults.reduce((sum, r) => sum + Math.pow(r.accuracy - meanAccuracy, 2), 0) / cvResults.length
            );
    ...
    const bestModels = allEvaluations.reduce((acc, evaluation) => {
      ...
      return acc;
    }, {} as Record<string, ModelEvaluation>);
